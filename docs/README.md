...
| 六亲强弱 | 91% (10/11, det) | 真实(杨炎 gold;确定性引擎层,样本小) |
| 具体事件/年份 | 0% 开放式 / 40% MCQ | 真实(名人验证事件) |

> **口径说明**
> - ✅ 真实 = 有独立 / 权威 gold,数字可 cite;格局/忌神/旺衰是**确定性注入或 LLM-引擎一致性**,**不是 accuracy**(不证明对错),需 gold 才算准确率。
> - **「结构层 90%」目标框定**:仅指**有独立 gold 的维度**——排盘 100%、用神 90%、六亲 91%(det)均已 ≥90% 达成;格局/忌神(确定性注入)、旺衰(LLM-引擎一致性)**不计入 90% 承诺**;事件层是物理天花板。用可 cite 的真 gold,不拿循环数字当成就。
> - **六亲**记分板为确定性引擎层(det,绕过 LLM),最近运行 **10/11 = 91%**;但 n 偏小(杨炎 gold 可比较 case 仅 11),det / e2e-prose / n 不同口径历史在 **58–91%** 间浮动,详见 `bazi_ai_90_roadmap.md` 与 memory。扩验证规模是下一步(B 阶段)。
