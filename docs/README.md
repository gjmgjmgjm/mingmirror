...
| 六亲强弱 | 74% (29/39, det) | ✅ 真实(杨炎 gold,det 绕过 LLM) |
| 具体事件/年份 | 0% 开放式 / 40% MCQ | 真实(名人验证事件) |

> **口径说明**
> - ✅ 真实 = 有独立 / 权威 gold,数字可 cite;格局/忌神/旺衰是**确定性注入或 LLM-引擎一致性**,**不是 accuracy**(不证明对错),需 gold 才算准确率。
> - **「结构层 90%」目标框定**:仅指**有独立 gold 的维度**——排盘 100%、用神 90% 已达成;**六亲 74%(det,n=39)未达 90%,是结构层唯一 accuracy 短板**(剩余 10 错例:engine 透干有根高估缺耗泄判定 + 坏根 over-fire,约半为 gold 噪声,边际递减)。格局/忌神(确定性注入)、旺衰(LLM-引擎一致性)**不计入**;事件层是物理天花板。
> - **六亲** det 全量 n=39,准确率 **74%**(10 错例:6 个 engine 见透干有根判强、缺"星被耗泄/夺食→弱"判定;4 个坏根 over-fire vs gold 主观分歧)。e2e deepseek prose 口径 ~73–77%。详见 `bazi_ai_90_roadmap.md` 与 memory。
