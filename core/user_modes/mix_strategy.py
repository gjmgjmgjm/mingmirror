...
class MixUserModeStrategy(BaseUserModeStrategy):
    ...
    async def collect_items(self, sec_uid: str, user_info: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        logger.warning("Mix API returned empty, attempting browser fallback")
        fallback_items = await self._fallback_mix_items(sec_uid)
        if fallback_items:
            return fallback_items

        return []

    async def _fallback_mix_items(self, sec_uid: str) -> List[Dict[str, Any]]:
        ...
        if not aweme_ids:
            logger.warning("Browser fallback for mix returned no aweme_id")
            return []
