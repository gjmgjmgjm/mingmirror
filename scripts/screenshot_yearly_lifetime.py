#!/usr/bin/env python3
"""Screenshot the YearlyChart page in lifetime mode."""
import asyncio
...
async def main():
    async with async_playwright() as p:
        ...
        await page.fill("#manualBazi", "庚午 辛巳 庚辰 壬午")
        ...
        # Switch to lifetime mode
        await page.click("button:has-text('看到80岁')")
        await page.click("button:has-text('生成流年精排')")
        await page.wait_for_selector("text=大运主题", timeout=90000)
        await asyncio.sleep(2)

        await page.screenshot(path="screenshots/yearly_lifetime.png", full_page=True)
        print("saved screenshots/yearly_lifetime.png")
        await browser.close()
