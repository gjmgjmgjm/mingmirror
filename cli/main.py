...
async def _run_bazi_subcommand(args, config: ConfigLoader) -> None:
    """处理 --extract-bazi 与 --build-knowledge-base 子命令。"""
    try:
        from tools.bazi_cli import build_knowledge_base_for_directory, extract_bazi_for_directory
    except ImportError as exc:
        display.print_error(
            f"八字功能需要安装可选依赖 rapidocr-onnxruntime："
            f"\n  pip install rapidocr-onnxruntime --no-deps\n原始错误：{exc}"
        )
        return

    bazi_cfg = config.get("bazi_extract") or {}
    base_path = Path(config.get("path") or "./Downloaded/")

    # Resolve target user directory.
    if args.extract_bazi:
        user_dir = Path(args.extract_bazi)
    elif args.build_knowledge_base:
        user_dir = Path(args.build_knowledge_base)
    else:
        user_dir = base_path

    # If a file-level path was passed (e.g. config.path), resolve author subdirs.
    if user_dir == base_path and not (user_dir / "post").exists():
        author_dirs = [d for d in user_dir.iterdir() if d.is_dir() and (d / "post").exists()]
        if not author_dirs:
            display.print_error(f"未找到可用的用户目录：{user_dir}")
            return
        targets = author_dirs
    else:
        targets = [user_dir]

    duration = args.bazi_duration if args.bazi_duration is not None else int(bazi_cfg.get("duration", 60))
    interval = args.bazi_interval if args.bazi_interval is not None else float(bazi_cfg.get("interval", 2.0))
    glossary_path = Path(args.bazi_glossary or bazi_cfg.get("glossary") or "bazi_glossary.json")
    output_dir = Path(args.bazi_output or bazi_cfg.get("output_dir") or "./bazi_knowledge")

    if args.extract_bazi is not None:
        display.print_info("正在初始化八字 OCR 模型...")
        try:
            from rapidocr_onnxruntime import RapidOCR

            ocr = RapidOCR()
        except Exception as exc:  # pragma: no cover
            display.print_error(f"OCR 模型初始化失败：{exc}")
            return

        for target in targets:
            display.print_info(f"正在提取八字：{target}")
            ...
            display.print_success(
                f"完成：总计 {summary['total']}，成功 {summary['success']}，"
                f"失败 {summary['failed']}，跳过 {summary['skipped']}"
            )
            display.print_info(f"清单保存至：{summary['manifest_path']}")

    if args.build_knowledge_base is not None:
        output_dir.mkdir(parents=True, exist_ok=True)
        for target in targets:
            name = target.name or "knowledge"
            output_path = output_dir / f"{name}_knowledge.md"
            if args.bazi_output:
                output_path = Path(args.bazi_output)
            display.print_info(f"正在构建知识库：{target}")
            ...
            display.print_success(
                f"完成：{summary['processed_count']} 个案例写入 {summary['output_path']}"
            )
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="USER_DIR",
        help="对下载目录中的视频 OCR 提取八字（可指定用户目录，默认使用 config.path）",
    )
    parser.add_argument(
        ...
        default=None,
        metavar="USER_DIR",
        help="根据已识别的八字 manifest 生成知识库（可指定用户目录）",
    )
    parser.add_argument("--bazi-duration", type=int, default=None, help="八字 OCR 只扫描前 N 秒（默认 60）")
    parser.add_argument("--bazi-interval", type=float, default=None, help="八字 OCR 采样间隔（默认 2.0 秒）")
    parser.add_argument("--bazi-glossary", type=str, default=None, help="纠错词表路径（默认 bazi_glossary.json）")
    parser.add_argument("--bazi-output", type=str, default=None, help="知识库输出文件路径")
    parser.add_argument(
        "--analyze-bazi",
        type=str,
        default=None,
        metavar="BAZI",
        help="用 DeepSeek 分析一个八字，例如：--analyze-bazi '甲子 丙寅 戊辰 庚午'",
    )
    parser.add_argument(
        "--bazi-question",
        type=str,
