...
def transcribe_file(
    video_path, model, ffmpeg_path, output_formats, language, converter, output_dir=None, prompt=None
):
    ...
    try:
        ...
        # Step 2: Whisper 识别
        transcribe_kwargs = {"language": language, "verbose": False}
        if prompt:
...
def main():
    ...
    parser.add_argument("--force", action="store_true", help="强制重新转录，覆盖已有 transcript")
    parser.add_argument("--prompt", default=None, help="Whisper initial_prompt 文本")
    parser.add_argument("--prompt-file", dest="prompt_file", default=None, help="从文件读取 Whisper initial_prompt")
    ...
    elif initial_prompt is None and Path("bazi_prompt.txt").exists():
        initial_prompt = Path("bazi_prompt.txt").read_text(encoding="utf-8").strip()
    ...
    if args.file:
        ...
        videos = find_videos(args.dir, skip_existing=args.skip_existing and not args.force, output_dir=args.output)
