...
| 六亲强弱 | 74% (29/39 det) | ✅ 真实(杨炎 gold,det) |
| 具体事件/年份 | 0% 开放式 / 40% MCQ | 真实(名人) |

> **「结构层 90%」= 有独立 gold 的维度**:排盘 100%、用神 90%(大 n)达成;**六亲 74%(det,n=39)未达 90%,是结构层唯一 accuracy 短板**。格局/忌神(确定性注入)、旺衰(LLM-引擎一致性)**非 accuracy,不计入**。六亲剩余 10 错例约半为 gold 噪声(memory 评估边际递减);事件层开放式 ≈0% 是物理天花板,产品输出**趋势**而非断言。
...
**下一步杠杆(按性价比)**:① ✅ 用神已扩到大 n=92(90.2%);② 旺衰无独立 gold(杨炎=六亲断象、MingLi/celebrity=事件),要谈 accuracy 需先标注命主旺衰 gold(数据工作,非代码攻坚);③ 六亲 det 74% 是真短板,但剩余错例需"星被耗泄/夺食→弱"判定(engine 现缺),约半为 gold 噪声、边际递减、有 regression 风险;④ 现阶段不建议做 SFT/LoRA(需 GPU,真人路线未必需要)。
