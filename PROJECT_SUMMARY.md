...
> 六亲记分板为确定性引擎层(最近 10/11=91%,但 n 偏小),det/e2e 口径历史在 58–91% 间浮动;事件层开放式 ≈0% 是物理天花板,产品输出**趋势**而非断言。
...
**下一步杠杆(按性价比)**:① 扩验证规模(n=12/30 噪声大);② 旺衰 75%→90%(结构层最弱项);③ 六亲接近 det 天花板,边际递减;④ 现阶段不建议做 SFT/LoRA(需 GPU,真人路线未必需要)。
